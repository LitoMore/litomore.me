...
分析下来我会觉得 pnpm 11 的每个功能模块之间有些割裂，虽然上了很多不错的功能，但到了实际开发情境下又显得不太得劲儿。接下来我应该会尝试给 pnpm 提一版代码，沿着 [pnpm/pnpm#11668] 里提到的方向优化这块体验。比起让 `audit --fix` 或 `install` 自动做太多事情，我更想先尝试一个显式的维护入口，比如增加一个 `pnpm clean` 命令，专门用来清理这类已经不再需要的 `minimumReleaseAgeExclude` 条目。这样清理动作是可预期的，也不会和安全修复本身混在一起。
...
[pnpm/pnpm#11668]: https://github.com/pnpm/pnpm/issues/11668
